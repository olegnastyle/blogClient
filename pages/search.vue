...
const posts = ref([])
const base_url = 'https://a1c537287dd6.vps.myjino.ru'
...
async function searchAlert() {
    ...
    if(!search.searchQuery ==! true) {
        try {
            const response = await $fetch(`https://a1c537287dd6.vps.myjino.ru/api/posts?populate=*&filters[$or][0][title][$containsi]=${search.searchQuery}&filters[$or][1][body][$containsi]=${search.searchQuery}`);
